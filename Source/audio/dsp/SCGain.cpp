...
namespace dsp
{
	SCGain::SCGain(param::Param& _scGain) :
		scGain(_scGain),
		listening(false),
		maxPeak(0.f)
	{
	...
	void SCGain::reset() noexcept
	{
		listening.store(false);
	}

	void SCGain::setListening(bool e) noexcept
	{
		if (listening.load() == e)
			return;
		listening.store(e);
	...
	void SCGain::operator()(float* const* samples,
	...
	bool SCGain::isListening() const noexcept
	{
